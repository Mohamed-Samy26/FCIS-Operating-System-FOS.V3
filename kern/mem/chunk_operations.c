...
void free_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 tbl_state;
	uint32 t_val;
	for(uint32 i = sva; i<=endva;i+=PAGE_SIZE){
		pf_remove_env_page(e, i);
	}
	for(uint32 i = 0; i<e->page_WS_max_size;i++){
		tbl_state =env_page_ws_get_virtual_address(e,i);
		if (tbl_state >= sva && tbl_state < endva){
//			unmap_frame(e->env_page_directory,tbl_state);
...
//		cprintf("HERE 3 %x\n",i);
		p_tbl = NULL;
		tbl_state = get_page_table(e->env_page_directory,i,&p_tbl);
		if(tbl_state == TABLE_IN_MEMORY){
